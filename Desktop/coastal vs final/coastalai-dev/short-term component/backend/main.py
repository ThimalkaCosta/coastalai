...
BASE_DIR = Path(__file__).resolve().parent.parent  # new-coastal/
NOTEBOOK = BASE_DIR / "forecasting_gpt.ipynb"
...
app = FastAPI(title="New-Coastal API")

default_origins = [
    "http://localhost:5173",
    "http://127.0.0.1:5173",
]
configured_origins = [o.strip() for o in os.getenv("FRONTEND_ORIGINS", "").split(",") if o.strip()]
...
def download_file(filename: str):
    file_path = OUTPUT_FORECASTS_DIR / filename
    if not file_path.exists() or not file_path.is_file():
        raise HTTPException(status_code=404, detail="File not found")
    if not filename.endswith((".kml", ".csv", ".geojson")):
...
def run_notebook(request: ForecastRequest):
    ...
    # Pre-flight check for quick mode — give a clear error before even starting
    if run_mode == "quick":
        missing = []
    ...
    try:
        ...
        # Extract the most useful part of the papermill error
        error_msg = str(e)
        # Try to find the actual assertion/error line
    ...
    if not out_nb.exists():
        return JSONResponse(
            status_code=500,
            content={"error": "Notebook ran but output file was not produced.", "expected": str(out_nb)},
        )
...
def latest_results():
    ...
    if not files:
        return JSONResponse(status_code=404, content={"error": "No results yet. Run /api/run first."})
...
def latest_stats():
    ...
    # Count high risk zones from spike summary if available
...
@app.get("/api/latest-risk-data")
def latest_risk_data():
    """
    Reads the CSVs saved by the notebook's final save cell:
      - output_forecasts/predicted_30days.csv   → full 30-day env + event score
      - output_forecasts/spike_summary.csv       → spike events only
      - output_forecasts/env_conditions.json     → seasonal env means
    """
    import pandas as pd
    ...
    if p30_file.exists():
        try:
            ...
            print(f"Error reading predicted_30days.csv: {e}")
    ...
    if spike_file.exists():
        try:
            ...
            print(f"Error reading spike_summary.csv: {e}")
    ...
    if env_json.exists():
        try:
            ...
            print(f"Error reading env_conditions.json: {e}")
    else:
        # Fallback: read from the ERA5/CMEMS merged CSV
        env_file = DATA_CACHE / "final_era5_cmems_daily_features.csv"
        if env_file.exists():
            try:
                ...
                print(f"Error reading env fallback: {e}")
