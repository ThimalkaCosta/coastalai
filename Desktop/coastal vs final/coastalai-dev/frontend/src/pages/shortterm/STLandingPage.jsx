        ...
        <div className="lp-risk-inner">
          ...
          <div className="lp-download-section">
            <div className="lp-download-card">
              ...
              <a href={`${apiBase}/api/shortterm/download/predicted_30days.csv`} download="predicted_30days.csv" className="lp-download-btn">
                ↓ Download CSV
              </a>
            </div>
            <div className="lp-download-card">
              ...
              <a href={`${apiBase}/api/shortterm/download/spike_summary.csv`} download="spike_summary.csv" className="lp-download-btn">
                ↓ Download CSV
              </a>
            </div>
          </div>
    ...
    <div className="lp-chart-svg-wrap">
      <svg viewBox={`0 0 ${W} ${H}`} className="lp-chart-svg lp-chart-dark" onMouseLeave={() => setTooltip(null)}>
        <defs>
          ...
          <filter id="stGlowBlue" x="-20%" y="-20%" width="140%" height="140%">
        ...
        <rect x="0" y="0" width={W} height={H} rx="12" fill="url(#stChartBg)" />

        {leftTicks.map((v, i) => (
          <line key={i} x1={PAD.l} y1={yL(v)} x2={W - PAD.r} y2={yL(v)} stroke={GRID} strokeWidth="1" />
        ))}
        {days.map((d, i) => (
          (i === 0 || (i + 1) % 5 === 0 || i === n - 1) ? (
            <line key={i} x1={xScale(i)} y1={PAD.t} x2={xScale(i)} y2={PAD.t + CH} stroke="rgba(96,165,250,0.07)" strokeWidth="1" />
          ) : null
        ))}
        {leftTicks.map((v, i) => (
